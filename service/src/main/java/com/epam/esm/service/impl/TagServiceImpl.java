...
public class TagServiceImpl implements TagService {
    ...
    public TagDto findMostPopularTagOfUserWithHighestCostOfOrder() {
        ...
        for (Order order : usersOrders) {
            certificates.add(certificateRepository.findById(order.getCertificateId()).orElseThrow(() ->
                    new ServiceException(ERR_NO_SUCH_CERTIFICATES)));
        }
        List<Tag> tags = new ArrayList<>();
        for (Certificate certificate : certificates) {
            for (Tag tag : certificate.getTagNames()) {
                tags.add(tagRepository.findById(tag.getId()).orElseThrow(() ->
                        new ServiceException(TAG_NOT_FOUND)));
            }
        }
